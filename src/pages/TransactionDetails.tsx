...
const TransactionDetails: React.FC = () => {
    ...
    const fetchStudentInfo = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_URL}/transactions.get?transaction_id=${id}`,
          headers,
        );
  ...
  const txData: DataItem[] = [
    { label: 'ID', value: transactionData?.id },
    {
      ...
      value: formatDateFlexible(
        transactionData?.target_date + 'T00:00:00-06:00',
        { type: 'date', withTimezoneOffset: true },
      ),
    },
    {
      label: 'Total a pagar',
      value: formatCurrency(Number(transactionData?.total)),
    },
    { label: 'Saldo', value: formatCurrency(Number(transactionData?.balance)) },
    {
      label: '¿Revocada?',
      value: `${transactionData?.is_revoked ? 'Sí' : 'No'}`,
    },
    { label: '¿Pagada?', value: `${transactionData?.is_paid ? 'Sí' : 'No'}` },
    {
      label: '¿Finalizada?',
      value: `${transactionData?.is_finished ? 'Sí' : 'No'}`,
    },
    {
      label: 'Fecha de inicio',
      value: formatDateFlexible(transactionData?.started_at + '', {
        type: 'datetime',
    ...
    },
    {
      ...
      value:
        (!!transactionData?.finished_at &&
          formatDateFlexible(transactionData?.finished_at + '', {
            type: 'datetime',
            withTimezoneOffset: true,
          })) ||
        'N/a',
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-1">
          <div>
            ...
            {transactionData?.is_paid && (
              <p className="text-green-500 text-lg font-semibold">Pagada</p>
            )}
            {transactionData?.is_revoked && (
              <p className="text-red-500 text-lg font-semibold">Revocada</p>
            )}
            {!transactionData?.is_paid &&
              !transactionData?.is_revoked &&
              !transactionData?.is_finished && (
